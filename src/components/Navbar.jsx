...
const MenuIcon = () => (
  <svg width="22" height="14" viewBox="0 0 20 12" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="lg:hidden flex flex-1 justify-end items-center gap-3">
          ...
          <div
            className={`${
              !toggle ? "hidden" : "flex"
            } p-6 bg-[#0d120d] border border-white/10 shadow-2xl absolute top-20 right-0 mx-4 my-2 min-w-[160px] z-10 rounded-xl`}
          >
            <ul className="list-none flex justify-end items-start flex-col gap-4">
              {navLinks.map((link) => (
                <li
                  key={link.id}
                  className={`${
                    active === link.title ? "text-white" : "text-[#b0baa8]"
                  } hover:text-accent text-[16px] font-medium cursor-pointer whitespace-nowrap`}
                  onClick={() => {
                    setToggle(!toggle);
